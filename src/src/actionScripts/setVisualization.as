...
function setVisualizatrionTree():void{
	...
	for(var i:int=0;i<dropCanvas.numChildren;i++){
		if(Object(dropCanvas.getChildAt(i)).className=='objects'){
			...
			/* var x:XMLList=XMLList("<x.position name='x="+dropCanvas.getChildAt(i).x+"' >"+dropCanvas.getChildAt(i).x+"</x.position>");
			object.appendChild(x);
		    var y:XMLList=XMLList("<y.position name='y="+dropCanvas.getChildAt(i).y+"' >"+dropCanvas.getChildAt(i).y+"</y.position>");
			object.appendChild(y);
			var z:XMLList=XMLList("<z.position name='z="+0+"' >0</z.position>");
			object.appendChild(z);
			var color:XMLList=XMLList("<color></color>");
			object.appendChild(color); */
			description.appendChild(object);
